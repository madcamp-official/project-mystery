...
namespace Wake.Exploration
{
    ...
    public static class AmbientInspectableCatalog
    {
        private static readonly AmbientInspectableSpec[] Entries =
        {
            ...
            I("PROP_COFFEE","NEWS_LOUNGE","식어 버린 커피","향보다 탄 맛이 먼저 올라온다. 다니엘이 기사 마감 때마다 이것을 세 잔씩 마셨다는 직원의 푸념이 떠오른다.",R(.23f,.32f),Cell(0,1)),
            ...
            I("PROP_MASK","BALLROOM","갈라 가면","화려하지만 값싼 장식이다. 이블린이라면 의상 소품의 금박이 고르지 않다며 사용하지 않았을 것이다.",R(.78f,.34f),Cell(0,2)),
