...
namespace Wake.Exploration
{
    ...
    public static class AmbientInspectableCatalog
    {
        private static readonly AmbientInspectableSpec[] Entries =
        {
            ...
            I("PROP_COFFEE","NEWS_LOUNGE","식어 버린 커피","향보다 탄 맛이 먼저 올라온다. Daniel이 기사 마감 때마다 이것을 세 잔씩 마셨다는 직원의 푸념이 떠오른다.",R(.23f,.32f),Cell(0,1)),
            ...
            I("PROP_MASK","BALLROOM","갈라 가면","화려하지만 값싼 장식이다. Evelyn이라면 의상 소품의 금박이 고르지 않다며 사용하지 않았을 것이다.",R(.78f,.34f),Cell(0,2)),
            I("PROP_CLIPBOARD","ENGINE_CONTROL","정비 체크보드","기관실 교대자가 그린 배의 단면 옆에 커피 얼룩이 번져 있다. Owen은 '중요한 도면에는 절대 잔을 올리지 않는다'고 강조한다.",R(.35f,.42f),Cell(1,2)),
