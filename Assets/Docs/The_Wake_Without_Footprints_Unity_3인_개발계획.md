...
| 4 | `The_Wake_Without_Footprints_Production_Manual_KR.pdf` | 씬 완료 조건, 제작 플로우, 에셋 수량, Unity 데이터·이벤트 권장 규격, QA | `968707E604343418AF6E664FF674B30AAD7C9EA3556D46B3AA0E5BA0EEAA73A8` |
| 5 | 이 개발 계획 | 기준 자료를 통합한 Unity 구현 방식, 데이터 계약, 일정, QA 기준 | 본문 |

충돌 시 사건의 사실관계는 시나리오 PDF, 공간 연결은 구조도 PDF를 우선한다. CSV는 실행용 초안이므로 PDF와 충돌하면 CSV를 수정한다. 제작 매뉴얼은 원본 자료를 실행 규격으로 확장한 문서이므로, `제작 권장`으로 표시된 값은 팀 리소스와 플레이테스트 결과에 따라 조정할 수 있다. 구현 편의를 위해 시나리오의 범인·시간·장소를 바꾸지 않는다.

### 1.2 원본 확정값과 제작 권장값

| 구분 | 포함 내용 | 변경 권한 |
|---|---|---|
| 원본 확정값 | 사건 구조, 등장인물, 41개 씬의 목적·등장·단서·진입 조건·선택·결과, C-01-C-18, 주요 퍼즐, 엔딩, 선박 구조, 대사 CSV | 기획 승인 없이 변경 금지 |
...
| 화면 | 필수 입력 | 완료 출력 |
|---|---|---|
| 탐색 | 이동, 핫스폿 선택, 조사 취소 | 조사 상태, 단서 획득, 다음 목표 |
| 대화 | 진행, 선택지 이동·확정, 로그 | 선택 결과, Trust·플래그, 다음 노드 |
| 퍼즐 | 조작, 힌트, 초기화, 나가기 | 정답 여부, 힌트 단계, 해금 상태 |
| Evidence Board | 카드 선택, 연결·해제, 가설 제출 | 활성 가설, 논증 완료 이벤트 |
| 재구성 | 카드 배치, 시간·경로 조정, 제출 | 타임라인 정답과 최종 심문 조건 |
...
- 기준 PDF 3종과 대사 CSV를 저장소 내 관리 위치로 복사하고 Git LFS 여부 결정
- 장소 코드 사전과 화자 사전 확정
- CSV의 선택·조건·효과 열 정규화
- `Wake.*` 네임스페이스와 폴더 생성
...
1. 시나리오·선박 구조도·제작 매뉴얼 PDF와 대사 CSV를 저장소의 `Docs/Source` 또는 팀이 합의한 경로로 이동한다.
