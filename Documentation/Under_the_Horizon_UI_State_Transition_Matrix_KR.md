...
| Map 장소 노드 | 25개 레거시 노드를 호환 목적으로 씬에 유지 | 활성 레이어 지도는 24개 플레이 가능 장소를 카탈로그에서 런타임 생성 |
...
씬의 기존 Map 뷰포트에는 25개 Inspector 노드를 호환 목적으로 보존한다.
실제 플레이에 사용하는 레이어 지도는 `MapDeckCatalog`와
`CanonicalLocationCatalog.Playable`의 24개 장소를 런타임에 생성하며,
Deck 6의 6개 비사용 장소는 노드와 층 선택에 포함하지 않는다.
